...
--	end
    ...
    if not isClient() or getServerOptions():getBoolean("AllowDestructionBySledgehammer") then
        local forbidenAreas = {
          {name = "lockerArea", x1 = 1, x2 = 131, y1 = 0, y2 = 154},
          {name = "CC", x1 = 11102, x2 = 11403, y1 = 8113, y2 = 8313},
        }

        -- Check if player is in any forbidden area
        local playerX = playerObj:getX()
        local playerY = playerObj:getY()
        local isInForbiddenArea = false

        for _, area in ipairs(forbidenAreas) do
            if playerX >= area.x1 and playerX <= area.x2 and
               playerY >= area.y1 and playerY <= area.y2 then
                isInForbiddenArea = true
                break
            end
        end

        -- Only add destroy option if not in forbidden area
        if not isInForbiddenArea then
