...
--     end
    ...
    -- Check if player is already in restricted area when UI opens
    if self:isPlayerInRestrictedArea() then
        if ISCraftingUI and ISCraftingUI.instance then
            ISCraftingUI.instance:close();
        end
    end
end
...
function ISWidgetHandCraftControl:update()
    ...
    -- Check if player is in restricted area and close UI if so
    if self:isPlayerInRestrictedArea() then
        if ISCraftingUI and ISCraftingUI.instance then
            ISCraftingUI.instance:close();
        end
...
-- Check if player is in the restricted crafting area (CC area)
function ISWidgetHandCraftControl:isPlayerInRestrictedArea()
    ...
    -- Check if player is within the restricted area
    if playerX >= x1 and playerX <= x2 and playerY >= y1 and playerY <= y2 then
        return true;
    end
...
function ISWidgetHandCraftControl:onButtonClick(_button)
    if self.buttonCraft and _button==self.buttonCraft then
        -- Check if player is in restricted area before crafting
        if self:isPlayerInRestrictedArea() then
            self.player:Say("I cannot do crafting here");
            if ISCraftingUI and ISCraftingUI.instance then
                ISCraftingUI.instance:close();
            end
            return;
...
function ISWidgetHandCraftControl:startHandcraft(force)
    ...
    -- Check if player is in restricted area before starting handcraft
    if self:isPlayerInRestrictedArea() then
        self.player:Say("I cannot do crafting here");
        if ISCraftingUI and ISCraftingUI.instance then
            ISCraftingUI.instance:close();
...
-- Check if any player is in restricted area every minute and make them say it
-- local function checkRestrictedAreaEveryMinute()
...
--     -- Check if player is within the restricted area
--     if playerX >= x1 and playerX <= x2 and playerY >= y1 and playerY <= y2 then
--         player:Say("I cannot do crafting here");
...
-- Events.EveryOneMinute.Add(checkRestrictedAreaEveryMinute);
