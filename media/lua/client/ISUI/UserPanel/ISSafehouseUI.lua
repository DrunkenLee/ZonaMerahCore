...
ISSafehouseUI.ReceiveSafehouseInviteZMPARTY = function(safehouse, host)
    -- Initialize the invite dialogs table if it doesn't exist
    ISSafehouseUI.inviteDialogs = ISSafehouseUI.inviteDialogs or {}

    -- Fix: Use host parameter instead of hostName variable
    if ISSafehouseUI.inviteDialogs[host] then
        ISSafehouseUI.inviteDialogs[host]:removeFromUIManager()
        ISSafehouseUI.inviteDialogs[host] = nil
    end

    -- Create the modal with explicit positioning
    local width = 350
    local height = 150
    local x = getCore():getScreenWidth() / 2 - width / 2
    local y = getCore():getScreenHeight() / 2 - height / 2

    -- Fix: Use host parameter in getText function
    local modal = ISModalDialog:new(
        x, y, width, height,
        getText("IGUI_ZMPARTY_Invitation", host),
        true, nil, ISSafehouseUI.onAnswerSafehouseInvite
    )
    ...
    if ISSafehouseUI.inviteDialogs then
        ISSafehouseUI.inviteDialogs[host] = modal
    end

    -- Debug output
    print("Safehouse invitation dialog created from: " .. host)
    return true
end

function removePlayerFromSafehouse(safehouseName, playerName)
    -- Input validation
    if not safehouseName or safehouseName == "" then
        print("ERROR: Please provide a safehouse name")
        print("Usage: removePlayerFromSafehouse('Base Alpha', 'PlayerName')")
        return false
    end

    if not playerName or playerName == "" then
        print("ERROR: Please provide a player name")
        print("Usage: removePlayerFromSafehouse('Base Alpha', 'PlayerName')")
        return false
    end

    -- Try to find the safehouse by title
    local safehouse = findSafehouseByTitle(safehouseName)
    if not safehouse then
        print("ERROR: Safehouse '" .. safehouseName .. "' not found")
        return false
    end

    -- Check if player is in this safehouse
    local playerList = safehouse:getPlayers()
    local playerInSafehouse = false
    for i = 0, playerList:size()-1 do
        if playerList:get(i) == playerName then
            playerInSafehouse = true
            break
        end
    end

    if not playerInSafehouse then
        print("ERROR: Player '" .. playerName .. "' is not a member of safehouse '" .. safehouseName .. "'")
        return false
    end

    -- Check if player is the owner (can't remove owner)
    if safehouse:getOwner() == playerName then
        print("ERROR: Cannot remove the owner of the safehouse. Use forceReleaseSafehouse instead.")
        return false
    end

    -- Kick player out of safehouse area if online and inside
    if not getServerOptions():getBoolean("SafehouseAllowTrepass") then
        local players = getOnlinePlayers()
        for i = 0, players:size()-1 do
            local player = players:get(i)
            if player:getUsername() == playerName then
                -- Check if player is in safehouse boundaries
                if player:getX() >= safehouse:getX() - 1 and
                   player:getX() < safehouse:getX2() + 1 and
                   player:getY() >= safehouse:getY() - 1 and
                   player:getY() < safehouse:getY2() + 1 then
                    print("Kicking player out of safehouse area...")
                    safehouse:kickOutOfSafehouse(player)
                end
                break
            end
        end
    ...
    print("Removing player '" .. playerName .. "' from safehouse '" .. safehouseName .. "'...")
    safehouse:removePlayer(playerName)

    -- Verify removal
    playerInSafehouse = false
    playerList = safehouse:getPlayers() -- Get updated list
    for i = 0, playerList:size()-1 do
        if playerList:get(i) == playerName then
            playerInSafehouse = true
            break
    ...
    if not playerInSafehouse then
        print("SUCCESS: Player '" .. playerName .. "' has been removed from safehouse '" .. safehouseName .. "'")
        return true
    else
        print("WARNING: Player might not have been removed properly")
        return false
    end
end

function forceReleaseSafehouse(safehouseName)
    -- Input validation
    if not safehouseName or safehouseName == "" then
        print("ERROR: Please provide a safehouse name")
        print("Usage: forceReleaseSafehouse('Base Alpha')")
        return false
    end
    ...
    print("Found safehouse '" .. safehouseName .. "' owned by '" .. ownerName .. "'")

    -- Force remove the safehouse
    print("Removing safehouse...")
    safehouse:removeSafeHouse(getPlayerFromUsername(ownerName))

    -- Verify removal
    if not findSafehouseByTitle(safehouseName) then
        print("SUCCESS: Safehouse '" .. safehouseName .. "' has been removed")
        return true
    else
        print("WARNING: Safehouse might not have been removed properly")
        return false
    end
end

function listAllSafehouses()
    local safehouses = SafeHouse.getSafehouseList()
    print("Available Safehouses (" .. safehouses:size() .. "):")
    for i = 0, safehouses:size()-1 do
        local sh = safehouses:get(i)
        print("  " .. i .. ": '" .. sh:getTitle() .. "' (Owner: " .. sh:getOwner() .. ")")
    end
end

function forceShowSafehouseInviteDialog(safehouse, hostName)
    if not safehouse then
        print("ERROR: No safehouse provided")
        return false
    end

    print("Creating safehouse invitation for: " .. safehouse:getTitle() .. " from: " .. hostName)
    ...
    local modal = ISModalDialog:new(
        x, y, width, height,
        getText("IGUI_SafehouseUI_Invitation", hostName),
        true, -- yes/no buttons
        nil,  -- target
        function(target, button) -- direct callback function
            if button.internal == "YES" then
                print("Player accepted invitation to " .. safehouse:getTitle())
                safehouse:addPlayer(getPlayer():getUsername())
                -- Trigger standard event
                triggerEvent("AcceptedSafehouseInvite", safehouse:getTitle(), hostName)
            else
                print("Player declined invitation")
            end
        end
    )
    ...
    -- Track the dialog if needed
    if ISSafehouseUI.inviteDialogs then
        ISSafehouseUI.inviteDialogs[hostName] = modal
    end

    return modal
end

ISSafehouseUI.ForceReceiveSafehouseInvite = function(safehouse, host)
    -- Initialize the invite dialogs table if it doesn't exist
    ISSafehouseUI.inviteDialogs = ISSafehouseUI.inviteDialogs or {}
    ISSafehouseUI.originalSafehouses = ISSafehouseUI.originalSafehouses or {}
    ...
    -- Debug output
    print("Attempting to force safehouse invitation from: " .. host)
    ...
    -- -- Temporarily remove player from current safehouse
    -- if originalSafehouse then
    --     print("Temporarily removing player from safehouse: " .. originalSafehouse:getTitle())
    --     -- originalSafehouse:removePlayer(username)
    --     -- Store for later
    --     ISSafehouseUI.originalSafehouses[username] = originalSafehouse
    -- end
    ...
    local modal = ISModalDialog:new(
        x, y, width, height,
        getText("IGUI_SafehouseUI_Invitation", host),
        true, nil, ISSafehouseUI.onAnswerForcedSafehouseInvite
    )
...
ISSafehouseUI.onAnswerForcedSafehouseInvite = function(target, button)
    local host = button.parent.host
    ...
    if button.internal == "YES" then
        -- Player accepted the invitation
        print("Player accepted invitation to " .. button.parent.safehouse:getTitle())
        button.parent.safehouse:addPlayer(username)
        -- Clear the original safehouse record
        ISSafehouseUI.originalSafehouses[username] = nil
        -- Trigger the normal event for compatibility
        triggerEvent("AcceptedSafehouseInvite", button.parent.safehouse:getTitle(), host)
    else
        -- Player declined, restore to original safehouse if they had one
        local originalSafehouse = ISSafehouseUI.originalSafehouses[username]
        if originalSafehouse then
            print("Restoring player to original safehouse: " .. originalSafehouse:getTitle())
            originalSafehouse:addPlayer(username)
            -- Clear the record
            ISSafehouseUI.originalSafehouses[username] = nil
        end
    end
