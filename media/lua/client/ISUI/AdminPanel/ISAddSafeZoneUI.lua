...
-------------------------------------------------
-------------------------------------------------
function setSafehouseData(_title, _owner, _x, _y, _w, _h)
...
function checkPlayerSafehouse(username)
    ...
    if not safehouseList or safehouseList:size() == 0 then
        print("No safehouses found on the server")
        return
    end

    print("Checking safehouses for player: " .. username)
    local foundSafehouse = false
    ...
    for i = 0, safehouseList:size()-1 do
        ...
        if safehouse:getOwner() == username then
            print("Player is OWNER of safehouse: " .. (safehouse:getTitle() or "Safehouse"))
            print("  Location: " .. safehouse:getX() .. "," .. safehouse:getY())
            foundSafehouse = true
        end

        -- Check if player is in the members list
        local players = safehouse:getPlayers()
        for j = 0, players:size()-1 do
            if players:get(j) == username then
                print("Player is MEMBER of safehouse: " .. (safehouse:getTitle() or "Safehouse"))
                print("  Owner: " .. safehouse:getOwner())
                print("  Location: " .. safehouse:getX() .. "," .. safehouse:getY())
                foundSafehouse = true
            end
        end
    ...
    if not foundSafehouse then
        print("Player does not have any safehouse")
    end

    return foundSafehouse
end

function addPlayerToSafehouseViaInvite(safehouse, inviter, inviteeUsername)
  -- This is how the current game does it
  sendSafehouseInvite(safehouse, inviter, inviteeUsername);

  -- Show a confirmation modal if desired
  local modal = ISModalDialog:new(0, 0, 350, 150,
    getText("IGUI_FactionUI_InvitationSent", inviteeUsername),
    false, nil, nil);
  modal:initialise()
...
function invitePlayerToNamedSafehouse(safehouseTitle, inviteeUsername)
    local playerObj = getSpecificPlayer(0)
    local safehouseList = SafeHouse.getSafehouseList()
    local targetSafehouse = nil

    -- Find the safehouse by title
    for i = 0, safehouseList:size()-1 do
        local safehouse = safehouseList:get(i)
        if safehouse:getTitle() == safehouseTitle then
            targetSafehouse = safehouse
            break
        end
    end

    if targetSafehouse then
        if targetSafehouse:isOwner(playerObj) then
            addPlayerToSafehouseViaInvite(targetSafehouse, playerObj, inviteeUsername)
            return true
        else
            print("You must be the owner of this safehouse to invite players")
        end
    else
        print("Safehouse not found: " .. safehouseTitle)
    end
...
function findSafehouseByTitle(title)
    ...
    for i = 0, safehouseList:size()-1 do
        local safehouse = safehouseList:get(i)
        if safehouse:getTitle() == title then
            return safehouse
        end
    end
    return nil
